...
def main():
    ...
    st.write("*Upload an image or use your webcam for continuous e-waste detection**(For better predction i suggest you to upload images)**.*")
    ...
    with tab2:
        ...
        # Capture frame
        if capture and 'current_detections' in st.session_state:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            cv2.imwrite(f"captured_{timestamp}.jpg", frame)
            st.success(f"Frame captured at {timestamp}")

        # Live result panel
        if 'current_detections' in st.session_state and st.session_state.current_detections:
